...
import {
  temperatureToColor,
  interpolateTemperaturePhysicsWithCircularBlending,
} from "@/lib/temperature-map/temperature";
import {
  computeDistanceGridAsync,
  isPointInsideBoundary,
} from "@/lib/temperature-map/distance";
...
const generateRenderCacheKey = (
  ...
  const sensorsKey = sensors
    .map((s) => `${s.x},${s.y},${s.temp},${s.label}`)
    .join("|");
  const wallsKey = walls
    .map((w) => `${w.x1},${w.y1},${w.x2},${w.y2}`)
    .join("|");
  const dimensionsKey = `${dimensions.width}x${dimensions.height}`;
  const colorKey = `${colorSettings.comfort_min_temp},${colorSettings.comfort_max_temp},${colorSettings.ambient_temp}`;
...
export const TemperatureMapCard = ({
  hass,
  config,
  previewMode,
  editMode,
}: ReactCardProps<Config>) => {
  useSignals();
  ...
  useEffect(() => {
    ...
    // But allow preview mode to show temperature heatmap for demo purposes
    if (isEditMode) {
    ...
    const cacheKey = generateRenderCacheKey(
      ...
      debouncedComputationConfig.walls,
      debouncedComputationConfig.dimensions,
      { comfort_min_temp, comfort_max_temp, ambient_temp },
    ...
    debouncedComputationConfig,

    comfort_min_temp,
    comfort_max_temp,
    ambient_temp,
    show_sensor_names,
    show_sensor_temperatures,
    isPreviewMode,
    isEditMode,
    drawOverlay,
  ]);
