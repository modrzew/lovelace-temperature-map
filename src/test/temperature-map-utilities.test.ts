...
import {
  ...
  lineIntersectsWalls,
  checkWallProximity,
} from "@/lib/temperature-map/geometry";
import {
  temperatureToColor,
  interpolateTemperaturePhysics,
} from "@/lib/temperature-map/temperature";
...
describe("Temperature and Color Processing", () => {
  describe("temperatureToColor", () => {
    ...
    it("should return blue for temperatures far below too cold threshold", () => {
      // Temperatures at exactly comfortMin are in transition zone, test far below for blue gradient
      expect(temperatureToColor(15, comfortMin, comfortMax)).toBe(
        "rgb(0, 0, 239)",
      ); // Far below = light blue
      expect(temperatureToColor(10, comfortMin, comfortMax)).toBe(
        "rgb(0, 0, 160)",
      ); // Very far below = darker blue
    });

    it("should return red for temperatures far above too warm threshold", () => {
      // Temperatures at exactly comfortMax are in transition zone, test far above for red gradient
      expect(temperatureToColor(29, comfortMin, comfortMax)).toBe(
        "rgb(246, 0, 0)",
      ); // Far above = light red
      expect(temperatureToColor(35, comfortMin, comfortMax)).toBe(
        "rgb(189, 0, 0)",
      ); // Very far above = darker red
    });

    it("should show smooth transitions at threshold boundaries", () => {
      // Test transition zones around comfortMin (18°C) and comfortMax (26°C)
      const atTooCold = temperatureToColor(18, comfortMin, comfortMax);
      const atTooWarm = temperatureToColor(26, comfortMin, comfortMax);

      // At exactly comfortMin/comfortMax, should be in transition zones (not pure colors)
      expect(atTooCold).toMatch(/^rgb\(\d+, \d+, \d+\)$/);
      expect(atTooWarm).toMatch(/^rgb\(\d+, \d+, \d+\)$/);

      // Should not be pure blue or pure red at exact thresholds
      expect(atTooCold).not.toBe("rgb(0, 0, 255)");
      expect(atTooWarm).not.toBe("rgb(255, 0, 0)");
    });

    it("should interpolate colors within comfort zone", () => {
      // Test specific points in the gradient
      const midTemp = (comfortMin + comfortMax) / 2; // 22°C
      const result = temperatureToColor(midTemp, comfortMin, comfortMax);
    ...
    it("should produce smooth gradient transitions", () => {
      ...
      const colors = temps.map((temp) =>
        temperatureToColor(temp, comfortMin, comfortMax),
      );
    ...
    it("should handle very precise temperature values", () => {
      ...
      const result = temperatureToColor(preciseTemp, comfortMin, comfortMax);
      expect(result).toMatch(/^rgb\(\d+, \d+, \d+\)$/);
    });

    it("should ensure RGB values are integers", () => {
      const result = temperatureToColor(21.7, comfortMin, comfortMax);
      const rgb = result.match(/\d+/g)?.map(Number);
  ...
  describe("interpolateTemperaturePhysics", () => {
    ...
    it("should return sensor temperature when very close to sensor", () => {
      ...
      const result = interpolateTemperaturePhysics(
        12,
        12,
        sensors,
        mockGrid,
